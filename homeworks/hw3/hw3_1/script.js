...
// – За допомогою циклу while вивести в документ 20 блоків h1 c довільним текстом і індексом всередині.
 let y=0;
while (y < 10) {
    document.write(`<h1>Довільний текст ${y}</h1>`)
    y++
}
...
document.write(`<div>– користувачів зі статусом true</div>`)
...
document.write(`<div>– користувачів зі статусом false</div>`)
for (let user of users) {
    if (!user.status) {
        document.write(`<div>${user.name}</div>`)
    }
...
document.write(`<div>– користувачів, які старші за 30 років</div>`)
for (let user of users) {
    if (user.age>30) {
        document.write(`<div>${user.name}</div>`)
    }
